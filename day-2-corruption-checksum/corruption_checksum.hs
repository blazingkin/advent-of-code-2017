...
splitOnNewLines string = splitOn "\n" string

splitOnTabs string = splitOn "\t" string

toNum string_list = if string_list == []
                    then []
                    else (read (head string_list) :: Integer) : toNum (tail string_list)

splitEachOnTabs list = if list == []
                        then []
                        else toNum (splitOnTabs (head list)) : splitEachOnTabs (tail list)

processInput string = splitEachOnTabs (splitOnNewLines string)

toDifference list = if list == []
                    then []
                    else (greatest (head list)) - (least (head list)) : toDifference (tail list)

totalSum list = if list == []
                then 0
                else (head list) + totalSum (tail list)

least list = if list == []
                then 99999
                else if (least (tail list)) < (head list)
                     then least (tail list)
                     else head list

greatest list = if list == []
                then 0
                else if (greatest (tail list)) > (head list)
                     then greatest (tail list)
                     else head list